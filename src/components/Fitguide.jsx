        ...
        {['Men', 'Women', 'Kids'].map((label) => (
          <Link key={label} className="  relative group" to={links[label]}>
            <img
              ...
              src={images[label]}
            />
            <div className="bg-black absolute bottom-6 left-28 bg-opacity-50 inline-block transform transition-all delay-300  group-hover:scale-110 px-6 py-2 text-white">
              {label}
            </div>
          </Link>
        ))}
